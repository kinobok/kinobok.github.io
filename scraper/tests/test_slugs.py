...
def test_slug_generation_with_accents():
    # Reported bug case
    assert (
        generate_slug("Little Amélie or the Character of Rain", 2004)
    ...
    assert (
        generate_slug("L'Amant de Lady Chatterley", 2022) == "lamant-de-lady-chatterley"
    )
    # Exception (due to Letterboxd not recognizing French people, see: generate_slug)
    assert (
        generate_slug("Sacré Cœur : Son règne n'a pas de fin", 2025)
        == "sacre-cur-son-regne-na-pas-de-fin"
    )
